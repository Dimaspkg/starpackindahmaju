...
export default function FootWearHyphenPage() {
  ...
  const content = {
    ...
    desc: language === 'id' 
      ...
      : language === 'jp'
        ? '耐摩耗性、柔軟性、環境ストレスに優れた高耐久性メタリックフィニッシュ Tobacco と保護コーティングで、フットウェアのデザイン性を向上させます。'
        : language === 'zh'
    ...
    features: [
      {
        ...
        desc: language === 'id' ? 'Formula coating elastis menyesuaikan secara dinamis dengan tekukan sepatu tanpa mengalami retak atau lepas.' : language === 'jp' ? '弾性コーティング配合により、シューズ of 屈曲に追従し、ひび割れや剥がれを防ぎます。' : language === 'zh' ? '弹性涂层配方可随鞋面弯曲动态调整，不易开裂或脱落。' : 'Elastic coating formula behaves dynamically with shoe flex, resisting cracking or peeling.'
      },
      {
        title: language === 'id' ? 'Tahan Gesekan & Goresan' : language === 'jp' ? '耐摩耗性＆防傷' : language === 'zh' ? '防刮与耐磨损' : 'Scuff & Scratch Resistance',
        desc: language === 'id' ? 'Lapisan pelindung atas (topcoat) melindungi logo dan aksen TPU dari benturan dan gesekan eksternal.' : language === 'jp' ? '高品質なトップコートが、ロゴやTPUアクセントを外部の擦れや傷から守ります。' : language === 'zh' ? '优质的顶面保护漆可保护标志和 TPU 装饰饰面免受外部刮擦和磨损。' : 'Superior protective topcoat shields logos and TPU accents from everyday friction and minor impacts.'
      },
      {
